...
const names=['ParticleModel','BouncingBallsDemo'];
const esc=s=>s.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
function markup(name){
 assert(names.includes(name));
 const src=fs.readFileSync(path.join(root,'pcd/assets/examples',name+'.java'),'utf8');
 return `<!-- BEGIN PHYSICS CONTENT ${name} -->\n<p><a href="assets/examples/${name}.java" download>Scarica ${name}.java</a> · Java 17.</p>\n<details data-physics-example="${name}"><summary>${name}: sorgente completo verificato</summary>\n<pre data-physics-source="${name}" tabindex="0" role="region" aria-label="Sorgente Java ${name}"><code class="language-java">${esc(src)}</code></pre>\n</details>\n<!-- END PHYSICS CONTENT ${name} -->`;
}
function main(){
 const original=fs.readFileSync(file,'utf8');let patch='';
 for(const name of names){
  const start=`<!-- BEGIN PHYSICS CONTENT ${name} -->`,end=`<!-- END PHYSICS CONTENT ${name} -->`;
 ...
 if(process.argv.includes('--check')){assert.equal(patch,'','Physics source drift');console.log('Two canonical Physics sources match visible chapter code');}
