...
(async()=>{
 ...
 try{for(const e of entries)for(const js of [true,false])for(const width of [1280,390]){
  ...
  if(js){for(const t of await p.locator('.lk-tabs').all())for(const [i,button] of (await t.locator(':scope > .lk-tablist > .lk-tab').all()).entries()){await button.click();assert(await t.locator(':scope > .lk-tabpanel').nth(i).isVisible());}
   ...
   const explorer=p.locator('#react-flow');await explorer.locator('.lk-se-trans button').filter({hasText:'LLM processes'}).click();assert.equal(await explorer.locator('.lk-se-cur b').textContent(),'Thought');
   await explorer.locator('.lk-se-trans button').filter({hasText:/^Safe decision/}).click();await explorer.locator('.lk-se-trans button').filter({hasText:'Return result'}).click();assert.equal(await explorer.locator('.lk-se-cur b').textContent(),'Safe Response');
   await explorer.getByRole('button',{name:'User Request',exact:true}).click();await explorer.locator('.lk-se-trans button').filter({hasText:'LLM processes'}).click();await explorer.locator('.lk-se-trans button').filter({hasText:'Unsafe decision'}).click();await explorer.locator('.lk-se-trans button').filter({hasText:'Data exfiltrated'}).click();assert.equal(await explorer.locator('.lk-se-cur b').textContent(),'Breach');
  }
