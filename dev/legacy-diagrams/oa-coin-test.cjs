...
print(json.dumps(rows))`],{encoding:'utf8',maxBuffer:2e6}));
 ...
 assert.equal(box.next(require('./oa-dm-content.cjs').next(require('./oa-selection-content.cjs').next(require('./oa-qq-content.cjs').next(c.next(before))))),html,'Only the reviewed coin, Q–Q, selection, DM and box/scatter transformations change the chapter');assert.equal(c.next(html),html);assert.equal(require('./oa-frequency-content.cjs').next(html),html);
 for(const i of [1,2,3,4,5,6,7,8,9]){const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>');assert.equal(html.match(re)[0],box.next(before).match(re)[0]);}
 const adjacent=s=>box.stripOldWidget(s.slice(s.indexOf('/* ---- Bespoke widget: mean vs median under skewness ---- */')).split('/* ---- Bespoke widget: is the coin rigged? ---- */')[0].split('</script>')[0]);assert.equal(adjacent(html),adjacent(before));
