...
print(json.dumps(rows))`],{encoding:'utf8',maxBuffer:2e6}));
 ...
 assert.equal(c.next(before),html,'Only the reviewed test sections, quiz and widget integration change');assert.equal(c.next(html),html);assert.equal(require('./oa-frequency-content.cjs').next(html),html);
 for(const i of [1,2,3,4,5,6,7,8,9,12,13,14]){const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>');assert.equal(html.match(re)[0],before.match(re)[0]);}
