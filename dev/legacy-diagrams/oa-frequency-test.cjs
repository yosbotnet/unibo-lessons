...
print(json.dumps(result))`],{input:JSON.stringify(m.data),encoding:'utf8'}));
 ...
 parse(html,{onParseError:e=>errors.push(e)});assert.deepEqual(errors,[]);assert.equal(center.next(box.next(require('./oa-dm-content.cjs').next(require('./oa-selection-content.cjs').next(require('./oa-qq-content.cjs').next(require('./oa-coin-content.cjs').next(c.next(before))))))),html);assert.equal(c.next(html),html);
 const svg=s=>[...s.matchAll(/<svg\b[\s\S]*?<\/svg>/g)].map(x=>x[0]);const existing=svg(before),now=svg(html);assert.equal(existing.length,8);assert.deepEqual(now.filter(x=>!x.includes('data-generated-plot="oa-center"')&&!x.includes('data-generated-plot="oa-symmetry"')&&!x.includes('data-frequency-chart')&&!x.includes('oa-coin-exact')&&!x.includes('oa-normal-qq')&&!x.includes('oa-dm-alignment')&&!x.includes('data-generated-plot="oa-box-')&&!x.includes('data-generated-plot="oa-pairing"')),existing.filter(x=>!x.includes('p86t')&&!x.includes('p87t')&&!x.includes('p88t')&&!x.includes('p82t')));
 ...
 for(let i=3;i<=9;i++){const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>'),old=center.next(box.next(before)).match(re)[0];assert.equal(html.match(re)[0],old);}
