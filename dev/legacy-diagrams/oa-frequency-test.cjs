...
print(json.dumps(result))`],{input:JSON.stringify(m.data),encoding:'utf8'}));
 ...
 parse(html,{onParseError:e=>errors.push(e)});assert.deepEqual(errors,[]);assert.equal(require('./oa-qq-content.cjs').next(require('./oa-coin-content.cjs').next(c.next(before))),html);assert.equal(c.next(html),html);
 ...
 for(let i=3;i<=14;i++){if([10,11,13].includes(i))continue;const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>'),old=before.match(re)[0];assert.equal(html.match(re)[0],i===12?c.wrapLegacyTables(old,'s'+i):old);}
 ...
 fs.writeFileSync('/home/ybc/notes-legacy-review-artifacts/oa-frequency-test.json',JSON.stringify({pins,slideSha256:crypto.createHash('sha256').update(source).digest('hex'),oracleCases:oracle.length,expected,markup:true,existingSVGsUnchanged:5,subsequentRevisions:['oa-coin-content.cjs','oa-qq-content.cjs'],scope:'Exact arithmetic and documented bin conventions; this histogram suite uses a stdlib oracle, not a NumPy/SciPy runtime'},null,2)+'\n');
 console.log('31 source observations, 24 rational cases, boundary/invalid tests, five unchanged SVGs and explicit coin/Q–Q compatibility passed');
