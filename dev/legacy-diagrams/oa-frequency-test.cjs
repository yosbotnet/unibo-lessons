...
print(json.dumps(result))`],{input:JSON.stringify(m.data),encoding:'utf8'}));
 ...
 parse(html,{onParseError:e=>errors.push(e)});assert.deepEqual(errors,[]);assert.equal(afterCenter(box.next(require('./oa-dm-content.cjs').next(require('./oa-selection-content.cjs').next(require('./oa-qq-content.cjs').next(require('./oa-coin-content.cjs').next(c.next(before))))))),html);assert.equal(c.next(html),html);
 const svg=s=>[...s.matchAll(/<svg\b[\s\S]*?<\/svg>/g)].map(x=>x[0]);const existing=svg(before),now=svg(html);assert.equal(existing.length,8);assert.deepEqual(now.filter(x=>!x.includes('data-generated-plot="oa-density"')&&!x.includes('data-generated-plot="oa-center"')&&!x.includes('data-generated-plot="oa-symmetry"')&&!x.includes('data-frequency-chart')&&!x.includes('oa-coin-exact')&&!x.includes('oa-normal-qq')&&!x.includes('oa-dm-alignment')&&!x.includes('data-generated-plot="oa-box-')&&!x.includes('data-generated-plot="oa-pairing"')),existing.filter(x=>!x.includes('p86t')&&!x.includes('p87t')&&!x.includes('p88t')&&!x.includes('p82t')));
 const tail=s=>{const pos=s.indexOf("/* ---- Bespoke widget: confidence intervals on the z-scale ---- */");assert(pos>=0);return s.slice(pos).split("/* ---- Bespoke widget: is the coin rigged? ---- */")[0].split("</script>")[0];};assert.equal(tail(html),tail(before),'Unrevised adjacent widget code remains byte-identical');
 for(let i=3;i<=9;i++){const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>'),old=afterCenter(box.next(before)).match(re)[0];assert.equal(html.match(re)[0],old);}
 ...
 fs.writeFileSync('/home/ybc/notes-legacy-review-artifacts/oa-frequency-test.json',JSON.stringify({pins,slideSha256:crypto.createHash('sha256').update(source).digest('hex'),oracleCases:oracle.length,expected,markup:true,existingSVGsUnchanged:4,subsequentRevisions:['oa-coin-content.cjs','oa-qq-content.cjs','oa-selection-content.cjs','oa-dm-content.cjs','oa-box-content.cjs','oa-center-content.cjs','oa-density-content.cjs'],scope:'Exact arithmetic and documented bin conventions; this histogram suite uses a stdlib oracle, not a NumPy/SciPy runtime'},null,2)+'\n');
 console.log('31 source observations, 24 rational cases, boundary/invalid tests, four unchanged SVGs and explicit subsequent-revision compatibility passed');
