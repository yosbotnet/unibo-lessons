...
print(json.dumps(result))`],{input:JSON.stringify(m.data),encoding:'utf8'}));
 ...
 const svg=s=>[...s.matchAll(/<svg\b[\s\S]*?<\/svg>/g)].map(x=>x[0]);const existing=svg(before),now=svg(html);assert.equal(existing.length,8);assert.deepEqual(now.filter(x=>!x.includes('data-generated-plot="oa-inference-')&&!x.includes('data-generated-plot="oa-density"')&&!x.includes('data-generated-plot="oa-center"')&&!x.includes('data-generated-plot="oa-symmetry"')&&!x.includes('data-frequency-chart')&&!x.includes('oa-coin-exact')&&!x.includes('oa-normal-qq')&&!x.includes('oa-dm-alignment')&&!x.includes('data-generated-plot="oa-box-')&&!x.includes('data-generated-plot="oa-pairing"')),existing.filter(x=>!x.includes('p83t')&&!x.includes('normal84-')&&!x.includes('normal85-')&&!x.includes('p86t')&&!x.includes('p87t')&&!x.includes('p88t')&&!x.includes('p82t')));
 ...
 fs.writeFileSync('/home/ybc/notes-legacy-review-artifacts/oa-frequency-test.json',JSON.stringify({pins,slideSha256:crypto.createHash('sha256').update(source).digest('hex'),oracleCases:oracle.length,expected,markup:true,existingSVGsUnchanged:1,subsequentRevisions:['oa-coin-content.cjs','oa-qq-content.cjs','oa-selection-content.cjs','oa-dm-content.cjs','oa-box-content.cjs','oa-center-content.cjs','oa-density-content.cjs','oa-inference-content.cjs'],scope:'Exact arithmetic and documented bin conventions; this histogram suite uses a stdlib oracle, not a NumPy/SciPy runtime'},null,2)+'\n');
 console.log('31 source observations, 24 rational cases, boundary/invalid tests, one unchanged taxonomy SVG and explicit subsequent-revision compatibility passed');
