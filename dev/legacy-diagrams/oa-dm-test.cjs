...
print(json.dumps(dict(model=ours.example(),sourceImplementationCompared=True,fractionalLossesChecked=24,swapAndScaleInvariance=True,numpy=numpy.__version__,scipy=scipy.__version__,pandas=pandas.__version__)))`,dir],{cwd:root,encoding:'utf8'}));assert.deepEqual(arithmetic.model,m,'Regenerate oa-dm-values.json from the Python model, not by hand');
 ...
 for(let i=1;i<=13;i++){const re=new RegExp('<section id="s'+i+'"[^>]*>[\\s\\S]*?</section>');assert.equal(html.match(re)[0],box.next(before).match(re)[0]);}
 assert.deepEqual(html.match(/<svg\b[\s\S]*?<\/svg>/g).filter(x=>!x.includes('oa-dm-alignment')),box.next(before).match(/<svg\b[\s\S]*?<\/svg>/g));assert.deepEqual(html.match(/<script\b[\s\S]*?<\/script>/g),box.next(before).match(/<script\b[\s\S]*?<\/script>/g));
