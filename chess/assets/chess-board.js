...
(function () {
  ...
  function draw(host, fen, arrows, marks, flip) {
    ...
    box.innerHTML = markup(fen, arrows, marks, flip);
  }
  function list(s) { return s ? s.split(',').map(function (x) { return x.trim(); }).filter(Boolean) : []; }
  function init(host) {
    var flip = host.dataset.flip === '1';
    if (!host.dataset.fens) { draw(host, host.dataset.fen, list(host.dataset.arrows), list(host.dataset.marks), flip); return; }
    var fens = host.dataset.fens.split('|'), sans = (host.dataset.sans || '').split('|'),
      arr = (host.dataset.arrows || '').split(';'), mks = (host.dataset.marks || '').split(';'), i = 0;
    ...
    function show() {
      draw(host, fens[i], list(arr[i]), list(mks[i]), flip);
      var txt = 'Start';
      if (i > 0) { var ply = i + (fens[0].split(' ')[1] === 'b' ? 1 : 0), n = Math.ceil(ply / 2); txt = n + (ply % 2 ? '. ' : '… ') + sans[i - 1]; }
      lab.textContent = txt + '  (' + i + '/' + (fens.length - 1) + ')';
      prev.disabled = i === 0; next.disabled = i === fens.length - 1;
    }
    prev.onclick = function () { if (i > 0) { i--; show(); } };
    next.onclick = function () { if (i < fens.length - 1) { i++; show(); } };
    show();
  }
  function boot() { Array.prototype.forEach.call(document.querySelectorAll('.cb'), init); }
