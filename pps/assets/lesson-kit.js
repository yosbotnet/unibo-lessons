...
(function (global) {
  ...
  function annotatedCode(sel, opts) {
    ...
    var expl = el('div', 'lk-ac-expl', (opts.lines[0] && opts.lines[0][1]) || 'Select a line for its explanation.');
    var btns = [];
  ...
  function stepper(sel, opts) {
    ...
    function render() {
      ...
      verdict.textContent = v && v.text ? v.text : (allDone ? 'Execution complete.' : 'Choose which process to advance.');
    }
    reset();
  }
