...
/* ===== politics course additions: shared drill + quiz renderers =====
   LessonKit.drill('hostId', ITEMS)  — ITEMS: [{q, opts:[..], a, fb}]
   LessonKit.quiz('hostId', QS, {chip:'id', verdict:'id', lesson:'Poli-XX'}) — same shape,
   right/wrong scoring with a sticky score chip and a final verdict line. */
(function (global) {
  'use strict';
  function mcqBox(idx, q) {
    var box = document.createElement('div'); box.className = 'mcq';
    var p = document.createElement('p'); p.className = 'mcq-q'; p.textContent = (idx + 1) + '. ' + q.q;
    box.appendChild(p);
    return box;
  }
  function drill(hostId, ITEMS) {
    var host = document.getElementById(hostId); if (!host) return;
    ITEMS.forEach(function (it, qi) {
      var box = mcqBox(qi, it);
      var ul = document.createElement('ul'); ul.className = 'mcq-opts';
      var fb = document.createElement('div'); fb.className = 'mcq-fb';
      it.opts.forEach(function (opt, oi) {
        var li = document.createElement('li'); var b = document.createElement('button'); b.type = 'button'; b.textContent = opt;
        b.addEventListener('click', function () {
          if (box.dataset.done) return; box.dataset.done = '1';
          var btns = ul.querySelectorAll('button'); btns.forEach(function (x) { x.disabled = true; });
          btns[it.a].classList.add('right'); if (oi !== it.a) b.classList.add('wrong');
          fb.textContent = (oi === it.a ? '✓ ' : '✗ ') + it.fb; fb.classList.add('on');
        });
        li.appendChild(b); ul.appendChild(li);
      });
      box.appendChild(ul); box.appendChild(fb); host.appendChild(box);
    });
  }
  function quiz(hostId, QS, opts) {
    opts = opts || {};
    var host = document.getElementById(hostId); if (!host) return;
    var chip = opts.chip ? document.getElementById(opts.chip) : null;
    var verdict = opts.verdict ? document.getElementById(opts.verdict) : null;
    var score = 0, answered = 0;
    function fmtScore() { return 'score: ' + score + ' / ' + QS.length; }
    function finish() {
      if (answered < QS.length || !verdict) return;
      verdict.style.display = 'block';
      verdict.innerHTML = 'Final: <strong>' + score + ' / ' + QS.length + '</strong>. ' + (score >= Math.ceil(QS.length * 0.66)
        ? 'That is midterm-MCQ standard on this topic. Bank it and move on.'
        : 'Below where you want it for now. Reread the sections the wrong answers point to, then come back: these six are the exact kind of definition-and-typology checks the written test uses.');
    }
    QS.forEach(function (qq, qi) {
      var box = mcqBox(qi, qq);
      var ul = document.createElement('ul'); ul.className = 'mcq-opts';
      var fb = document.createElement('div'); fb.className = 'mcq-fb';
      qq.opts.forEach(function (opt, oi) {
        ...
        b.addEventListener('click', function () {
          if (box.dataset.done) return; box.dataset.done = '1';
          var btns = ul.querySelectorAll('button'); btns.forEach(function (x) { x.disabled = true; });
          btns[qq.a].classList.add('right');
          if (oi === qq.a) { score += 1; fb.textContent = '✓ +1. ' + qq.fb; }
          else { b.classList.add('wrong'); fb.textContent = '✗ ' + qq.fb; }
          fb.classList.add('on'); answered++;
          if (chip) chip.textContent = fmtScore();
          finish();
        });
        li.appendChild(b); ul.appendChild(li);
      });
  ...
  if (global.LessonKit) { global.LessonKit.drill = drill; global.LessonKit.quiz = quiz; }
})(window);

/* BEGIN SHARED CONTENT LAYOUT */
